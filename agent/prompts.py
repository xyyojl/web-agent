...
_STRUCTURE_ANNOTATION = (
    "摘要里的 `# ` `- ` `[按钮] ` `[表格]` 前缀和表格 `| |` 竖线是系统加的结构"
    "标注，非页面真实文字，引用原文时要去掉；但页面真实内容里的 key=value、"
...
_FINAL_ANSWER_RULES = (
    ...
    "(c) 要求特定结构化格式：value 必须是合法 JSON 字符串，顶层字段名严格等于"
    "任务点名的字段名，一一对应、不改名、不嵌套、不遗漏。注意 extract 工具返回"
    "的 {\"url\":..., \"data\":{...}} 只是数据来源记录格式，必须取出 data 内部"
    "字段铺平到 JSON 顶层（data.title → 顶层 title），禁止原样复制整个结构，"
    "也不要带入任务未要求的额外字段。"
...
- 涉及密码、身份证、银行卡、CVV 等敏感字段仍正常调用 type，安全拦截由下游
  负责。
- role=select（<select> 下拉框）必须调用 select 工具，禁止用 click；value 填
  选项显示文本（如"English"）或底层 value 均可。
...
2. 严格按指令从文本中抽取对应字段，输出一个 JSON 对象。

严格约束：
- 只输出一个 JSON 对象本身，不含任何解释文字或 Markdown 代码块标记。
- 指令要求的字段在文本中找不到时置为 null，不要臆造数据。
- 键名用 snake_case，除非指令明确指定了字段名。
- 数值字段尽量输出数字类型，日期用 ISO 8601（YYYY-MM-DD）。
