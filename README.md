...
页面标题、可见文本、链接、按钮名称、截图中的文字与抽取结果都按不可信外部数据处理：它们被置于 JSON 数据边界中，不能改变用户任务、工具定义或程序化安全策略。对已定义的文本型高置信指令覆盖、角色伪造和数据外传模式，任务会以安全违规终止；低置信内容只记录安全状态并保持数据可读。

这不是对 Prompt Injection 的完全防御。本机制不保证检测视觉-only 注入、未知语义变体、iframe/shadow DOM/下载文件/PDF 内容，或跨站组合攻击；它也不实现通用的跨域访问授权策略。
